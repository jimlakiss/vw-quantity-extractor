# iQs VW Exporter V2.14 - Per-Component Quantities (Verified API)
# ═══════════════════════════════════════════════════════════════
...
#     GetDescriptionText(h)        → user-assigned description
#     GetObjectTags(h)             → VW tags array
#     IFC_GetIFCEntity(h)          → IFC type (IfcWall, IfcSlab, etc.)
#     IFC_GetIFCName(h)            → IFC name
#     GetPluginStyle(h)            → style name
#   Slab support (type 71):
#     GetSlabHeight(h)             → slab elevation
#     GetDatumSlabComponent(h)     → datum component index
#     GetSlabStyle(h)              → slab style
#     GetComponentAutoBoundEdgeOffset(h,i) → slab edge offsets
#     GetComponentManualEdgeOffset(h,i)    → slab edge offsets
#
...
def promote(obj):
    ...
    # ── Record field promotion (existing) ──
    for rec in recs:
    ...
    # ── V2.12: Build best available name ──
    # Priority: record name → ifc_name → description → style_name →
    #           wall_style → slab_style → labelText → vw_name
    if not obj.get("name"):
        # Wall style is the OIP name for walls (type 68)
        wall_style = None
        if obj.get("wall") and isinstance(obj["wall"], dict):
            wall_style = cv(obj["wall"].get("wall_style_api"))
        # Slab style for slabs (type 71)
        slab_style = None
        if obj.get("slab") and isinstance(obj["slab"], dict):
            slab_style = cv(obj["slab"].get("slab_style"))

        obj["name"] = (
            cv(obj.get("ifc_name")) or
            cv(obj.get("description")) or
            cv(obj.get("style_name")) or
            wall_style or
            slab_style or
            cv(p.get("labelText")) or
            cv(obj.get("vw_name"))
        )

    # ── V2.12: Build description from all sources ──
    # Combine description, ifc_entity, style_name, tags into a human-readable string
    if not obj.get("description"):
        parts = []
        if obj.get("ifc_entity"): parts.append(obj["ifc_entity"])
        if obj.get("style_name"): parts.append(obj["style_name"])
        if obj.get("pio"): parts.append(obj["pio"])
        if parts:
            obj["description"] = " · ".join(parts)
...
def slab_detail(h, t):
    """Extract slab-specific quantities. Type 71."""
    if t not in CFG["slab_types"]: return None
    s = {}

    # REAL = vs.GetSlabHeight(slab) → mm (Z position / elevation)
    if hv("GetSlabHeight"):
        sh = sc(vs.GetSlabHeight, h)
        if sh is not None:
            try: s["slab_height_mm"] = float(sh)
            except: pass

    # INTEGER = vs.GetDatumSlabComponent(object) → which component is datum
    if hv("GetDatumSlabComponent"):
        dc = sc(vs.GetDatumSlabComponent, h)
        if dc is not None:
            try: s["datum_component_idx"] = int(dc)
            except: pass

    # LONGINT = vs.GetSlabStyle(slab) → style index
    if hv("GetSlabStyle"):
        ss = sc(vs.GetSlabStyle, h)
        if ss is not None:
            try:
                si = int(ss)
                s["slab_style_idx"] = si
                # Try to resolve style name via Index2Name
                if hv("Index2Name"):
                    sn = sc(vs.Index2Name, si)
                    if isinstance(sn, (list,tuple)):
                        sn = next((v for v in sn if isinstance(v,str) and v.strip()), None)
                    if sn and isinstance(sn, str) and sn.strip():
                        s["slab_style"] = sn.strip()
            except: pass

    # Component thickness sum
    if hv("GetNumberOfComponents"):
        res = sc(vs.GetNumberOfComponents, h)
        n = 0
        if isinstance(res, tuple) and len(res) >= 2:
            n = res[1] if res[0] else 0
        ...
        if n > 0:
            ...
            total_thick = 0
            for i in range(1, n + 1):
                raw = sc(vs.GetComponentWidth, h, i)
                if isinstance(raw, tuple) and len(raw) >= 2:
                    ...
                    w = 0
                try: total_thick += float(w or 0)
                except: pass
            if total_thick > 0:
                s["thickness"] = total_thick
                s["thickness_src"] = "comp_sum"
...
def harvest(h):
    ...
    # ── V2.12: Object identification ──
    # Description text (user-assigned in OIP or via Data Manager)
    if hv("GetDescriptionText"):
        desc = sc(vs.GetDescriptionText, h)
        if isinstance(desc, (list,tuple)):
            desc = next((v for v in desc if isinstance(v,str) and v.strip()), None)
        if desc and isinstance(desc, str) and desc.strip() and desc.strip().lower() != "none":
            o["description"] = desc.strip()

    # Object tags (VW 2023+)
    if hv("GetObjectTags"):
        tags_raw = sc(vs.GetObjectTags, h)
        if tags_raw:
            # Returns (BOOLEAN, outArrTags) — unwrap
            if isinstance(tags_raw, (list,tuple)) and len(tags_raw) >= 2:
                ok_t, arr = tags_raw[0], tags_raw[1]
                if ok_t and arr:
                    if isinstance(arr, (list,tuple)):
                        tags = [str(t).strip() for t in arr if t and str(t).strip()]
    ...
    # IFC entity type (e.g. "IfcWall", "IfcSlab", "IfcSanitaryTerminal")
    if hv("IFC_GetIFCEntity"):
        ifc_ent = sc(vs.IFC_GetIFCEntity, h)
        if isinstance(ifc_ent, (list,tuple)):
            ifc_ent = next((v for v in ifc_ent if isinstance(v,str) and v.strip()), None)
        if ifc_ent and isinstance(ifc_ent, str) and ifc_ent.strip():
            o["ifc_entity"] = ifc_ent.strip()

    # IFC name (user-assigned IFC identifier)
    if hv("IFC_GetIFCName"):
        ifc_nm = sc(vs.IFC_GetIFCName, h)
        if isinstance(ifc_nm, (list,tuple)):
            ifc_nm = next((v for v in ifc_nm if isinstance(v,str) and v.strip()), None)
        if ifc_nm and isinstance(ifc_nm, str) and ifc_nm.strip():
            o["ifc_name"] = ifc_nm.strip()

    # Plugin style name (e.g. wall style, slab style, PIO style)
    if hv("GetPluginStyle"):
        ps = sc(vs.GetPluginStyle, h)
        if isinstance(ps, (list,tuple)):
            ps = next((v for v in ps if isinstance(v,str) and v.strip()), None)
        if ps and isinstance(ps, str) and ps.strip():
            o["style_name"] = ps.strip()
    try:
        ...
        if comps:
            ...
            # ── V2.12: Slab/roof offset-adjusted quantities ──
            # GetComponentNetArea does NOT account for edge offsets on slabs/roofs.
            # Neither does ObjPerim — it returns the base slab boundary for all components.
            #
            # Offset polygon geometry (convex polygon inset by distance d):
            #   A'(d) = A₀ - P₀·d + π·d²    (area shrinks as you inset)
            #   P'(d) = P₀ - 2π·d            (perimeter shrinks at corners)
            #   V'    = A' × thickness
            #
            # Sign convention: positive offset = INSET from edge (smaller)
            if t in (71, 84):
                areas = o.get("areas", {})
                base_area_mm2 = areas.get("area_2d") or 0
                perim_mm = areas.get("perim_2d") or 0

                if base_area_mm2 > 0 and perim_mm > 0:
                    for c in comps:
                        auto_off = c.get("auto_edge_offset") or 0
                        manual_off = c.get("manual_edge_offset") or 0
                        total_off = float(auto_off) + float(manual_off)

                        # Use component net_area as base if VW provides it, else parent
                        comp_base_mm2 = c.get("net_area_mm2") or base_area_mm2

                        if total_off != 0:
                            # Area: A' = A₀ - P₀·d + π·d²
                            adj_mm2 = comp_base_mm2 - perim_mm * total_off + math.pi * total_off * total_off
                            # Perimeter: P' = P₀ - 2π·d
                            adj_perim = perim_mm - 2.0 * math.pi * total_off

                            if adj_mm2 > 0:
                                c["offset_area_mm2"] = adj_mm2
                                c["offset_area_m2"]  = adj_mm2 / 1e6
                            if adj_perim > 0:
                                c["offset_perim_mm"] = adj_perim

                            # Debug: store inputs for verification
                            c["offset_base_area_mm2"] = comp_base_mm2
                            c["offset_base_perim_mm"] = perim_mm
                            c["offset_total_mm"]      = total_off

                            thick = c.get("thickness") or 0
                            if adj_mm2 > 0 and thick > 0:
                                c["offset_volume_mm3"] = adj_mm2 * thick
                                c["offset_volume_m3"]  = adj_mm2 * thick / 1e9
                        else:
                            # Zero offset — inherit parent perimeter and area if VW didn't provide
                            c["offset_perim_mm"] = perim_mm
                            if not c.get("net_area_mm2") and comp_base_mm2 > 0:
                                c["net_area_mm2"] = comp_base_mm2
                                c["net_area_m2"]  = comp_base_mm2 / 1e6
        if pd:
...
def iqs_push(jsonl_path):
    ...
    req = urllib.request.Request(url, data=payload, headers={
        "Content-Type":"application/json", "X-IQS-Exporter":"2.12"})
    try:
