# iQs VW Exporter V2.11 - Per-Component Quantities (Verified API)
# ═══════════════════════════════════════════════════════════════
# V2.10 adds per-component area/volume/length using VERIFIED API calls:
...
def promote(obj):
    ...
    # ── V2.10: Build best available name ──
    # Priority: record name → ifc_name → description → style_name → labelText → vw_name
    if not obj.get("name"):
        obj["name"] = (
    ...
    # ── V2.10: Build description from all sources ──
...
def get_components_raw(h, t):
    ...
    for i in range(1, n + 1):
        ...
        # ── V2.10: Slab component edge offsets ──
...
def iqs_push(jsonl_path):
    ...
    payload = json.dumps({
        "vw_import": {
            "project": project, "exporter_version": "2.10",
    ...
    req = urllib.request.Request(url, data=payload, headers={
        "Content-Type":"application/json", "X-IQS-Exporter":"2.10"})
    try:
...
def run():
    ...
    c = build_criteria()
    dbg("=== iQs VW Exporter V2.10 ==="); dbg(f"Criteria: {c}"); dbg(f"Export dir: {d}")
