# iQs VW Exporter V2.9 - Per-Component Quantities (Verified API)
# ═══════════════════════════════════════════════════════════════
# V2.8 adds per-component area/volume/length using VERIFIED API calls:
...
def promote(obj):
    ...
    # ── V2.8: Build best available name ──
    ...
    # ── V2.8: Build description from all sources ──
...
def get_components_raw(h, t):
    ...
    for i in range(1, n + 1):
        ...
        # ── V2.8: Slab component edge offsets ──
...
def iqs_push(jsonl_path):
    ...
    payload = json.dumps({
        "vw_import": {
            "project": project, "exporter_version": "2.8",
    ...
    req = urllib.request.Request(url, data=payload, headers={
        "Content-Type":"application/json", "X-IQS-Exporter":"2.8"})
    try:
...
def run():
    ...
    c = build_criteria()
    dbg("=== iQs VW Exporter V2.8 ==="); dbg(f"Criteria: {c}"); dbg(f"Export dir: {d}")
