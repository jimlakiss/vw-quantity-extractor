# iQs VW Exporter V2.13 - Per-Component Quantities (Verified API)
# ═══════════════════════════════════════════════════════════════
...
def harvest(h):
    ...
    try:
        ...
        if comps:
            ...
            # ── V2.12: Slab/roof offset-adjusted areas ──
            # GetComponentNetArea does NOT account for edge offsets on slabs/roofs.
            # We calculate the adjusted area using offset polygon geometry:
            #   A(d) = A₀ - P₀·d + π·d²
            # Sign convention assumption: positive offset = INSET from edge (smaller area)
            #   If this is backwards, flip the sign: A(d) = A₀ + P₀·d + π·d²
            if t in (71, 84):
                areas = o.get("areas", {})
                ...
                if base_area_mm2 > 0 and perim_mm > 0:
                    for c in comps:
                        ...
                        if total_off != 0:
                            # Offset polygon: A' = A₀ - P·d + π·d²
                            # Positive d = inset from edge (area decreases)
                            # Negative d = extends beyond edge (area increases)
                            ...
                            if adj_mm2 > 0:
                                ...
                                # Debug: store inputs so Jim can verify
                        ...
                        elif not c.get("net_area_mm2") and comp_base_mm2 > 0:
                            # No offset AND no VW area — inherit parent area
