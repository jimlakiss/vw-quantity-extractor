# iQs VW Exporter V2.7 - Per-Component Quantities
...
def wall_detail(h,t):
    ...
    if w.get("gross") and w.get("net"): w["openings"]=w["gross"]-w["net"]
    return w
def pio_dims(pn,pp):
...
def calc_uvw(t,pn,wd,pd,dims3d,bb2d,raw3d=None):
    ...
    if t in CFG["wall_types"] and wd:
        u["u"],u["v"],u["w"],u["m"]=wd.get("length"),wd.get("height"),wd.get("thickness"),"wall_api"
        return u
    if pd:
        pt=pd.get("pio_type")
        if pt=="FramingMember":
...
def get_components_raw(h, t):
    ...
    # ── Parent wall data for per-component calcs ──
    wall_height = None
    wall_length = None
    wall_gross  = None
    wall_net    = None
    if t in CFG["wall_types"]:
        wall_height = sr(vs.GetObjectVariableReal, h, 615)
        wall_gross  = sr(vs.GetObjectVariableReal, h, 608)
        wall_net    = sr(vs.GetObjectVariableReal, h, 611)
        if wall_gross and wall_height and wall_height > 0:
            wall_length = (wall_gross * 1e6) / wall_height  # back-calc length in mm
    ...
    for i in range(1, n + 1):
        ...
        # ══════════════════════════════════════════════════════════════
        # V2.7: Per-component quantity probing
        # Try every likely VW API for per-component area/volume/length.
        # sc() safe-call: if function doesn't exist → None, no crash.
        # ══════════════════════════════════════════════════════════════
        for api_name, key in [
            ("GetComponentArea",          "vw_area"),
            ("GetComponentNetArea",       "vw_net_area"),
            ("GetComponentGrossArea",     "vw_gross_area"),
            ("GetComponentVolume",        "vw_volume"),
            ("GetComponentNetVolume",     "vw_net_volume"),
            ("GetComponentGrossVolume",   "vw_gross_volume"),
            ("GetComponentLength",        "vw_length"),
            ("GetComponentNetLength",     "vw_net_length"),
            ("GetComponentGrossLength",   "vw_gross_length"),
            ("GetComponentHeight",        "vw_height"),
            ("GetComponentNetHeight",     "vw_net_height"),
            ("GetComponentWallArea",      "vw_wall_area"),
            ("GetComponentNetWallArea",   "vw_net_wall_area"),
        ]:
            if not hv(api_name):
                continue
            ok2, v = _unwrap_ok_val(sc(getattr(vs, api_name), h, i))
            if ok2 and v is not None:
                fv = sfz(v)
                if fv is not None and fv != 0:
                    c[key] = fv

        # ── Calculated: per-component quantities from parent wall ──
        # Always computed as fallback AND cross-check vs VW native values.
        # Units: eff_height in mm, areas in m², volumes in m³
        if t in CFG["wall_types"] and wall_height:
            top_off = sfz(c.get("top_offset"), default=0.0) or 0.0
            bot_off = sfz(c.get("bottom_offset"), default=0.0) or 0.0
            eff_h = wall_height - top_off - bot_off
            c["calc_eff_height"] = round(eff_h, 4)

            if wall_length and wall_length > 0:
                comp_gross = wall_length * eff_h / 1e6  # m²
                c["calc_gross_area"] = round(comp_gross, 6)

                # Net area: scale by parent gross/net ratio (openings)
                if wall_gross and wall_net and wall_gross > 0:
                    ratio = wall_net / wall_gross
                    c["calc_net_area"] = round(comp_gross * ratio, 6)
                    c["calc_opening_deduct"] = round(comp_gross * (1.0 - ratio), 6)

            thick = sfz(c.get("thickness"), default=0.0) or 0.0
            if thick > 0:
                if c.get("calc_gross_area"):
                    c["calc_gross_volume"] = round(c["calc_gross_area"] * thick / 1000.0, 6)
                if c.get("calc_net_area"):
                    c["calc_net_volume"] = round(c["calc_net_area"] * thick / 1000.0, 6)
...
def run():
    ...
    c = build_criteria()
    dbg("=== iQs VW Exporter V2.6 ==="); dbg(f"Criteria: {c}"); dbg(f"Export dir: {d}")
